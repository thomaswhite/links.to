	...
	*/
	...
	function updateTitle(content, reposition)
	{
		...
		// Append new content if its a DOM array and show it if hidden
			...
			elem.empty().append(content.show()); // css({ display: 'block' }) TW:prevent changing inline elements to BLOCK
		}
	...
	{
		var elem = elements.content, newContent = '';
		...
		// Use function to parse content
		if($.isFunction(content)) {
			newContent = content.call(target, cache.event, self);
			if( newContent === FALSE ){
				return FALSE;  // TW: no content to display.
			}else
				newContent = newContent || '';
		}
		// Append new content if its a DOM array and show it if hidden
		if(content && content.jquery && content.length > 0) {
			elem.empty().append(content.show()); //.css({ display: 'block' }) TW:prevent changing inline elements to BLOCK
		}else {
			// Content is a string, sanitize it first.
			// TW, CR19016 prevent XSS.
			var strippedContent = newContent || content;
			while( /\<script[^<]*<\/script>/gi.test(strippedContent) ){ // remove nested script tags
				strippedContent = strippedContent.replace(/\<script[^<]*<\/script>/gi, '?')
			};
			elem.html(strippedContent);
        }
	...
	function assignEvents()
	{
		...
		{
			if( tooltip.hasClass(disabled) && !tooltip.hasClass('tooltipEvenDisabled') ) {	return FALSE; }
	...
	*/
	$.extend(self, {
		render: function(show)
		{
			...
			if( FALSE === updateContent(options.content.text, FALSE)){
				self.rendered = TRUE;
				return self;
			};
			self.rendered = TRUE;
		...
		toggle: function(state, event)
		{
			...
			if(state) {
				...
				if($.isFunction(contentOptions.text)) {
					var updated = updateContent(contentOptions.text, FALSE);
					if( updated === FALSE ){ // TW: no content to display. Hide this tooltip
						tooltip.hide();
						return;
					}
				}
...
*/
			...
			tip.css(backgroundColor, transparent).css('border', '');
	...
	$.extend(self, {
		init: function()
		{
			...
			.bind('tooltipfocus'+globalNamespace, function(event) {
				...
				// Set overlay z-index
				overlay[0].style.zIndex = newIndex-1; // TW: keep the overlay just under the current qtip
...
PLUGINS.modal.zindex = QTIP.zindex + 5000;
