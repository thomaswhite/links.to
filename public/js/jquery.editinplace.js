...
$.fn.editInPlace = function(options) {
	...
	var settings = $.extend({}, $.fn.editInPlace.defaults, options, {id: new Date().getTime() });
...
$.fn.editInPlace.defaults = {
	...
	single_instance:    true, // when open the editor close all other edit in place editors  //TW
...
$.extend(InlineEditor.prototype, {
	...
	init: function() {
        ...
        //TW
        var thisEditor = this;
        $('body').delegate('*', 'close-all-eip-editors.eip', function(event, originatorID ){
            if( originatorID != thisEditor.settings.id ){
                if( thisEditor.dom.hasClass('editInPlace-active')){
                    thisEditor.restoreOriginalValue();
                    thisEditor.reinit();
                }
            }
        });
    ...
    closeOtherEditors: function(){
        if( this.settings.single_instance ){
            this.dom.trigger('close-all-eip-editors.eip', [this.settings.id]);
        }
    },
...
//        }else{
 ...
 //       };
	...
	createEditorElement: function() {
		...
		if ("select" === this.settings.field_type) {
			...
			editor = $('<input type="text" ' + this.inputNameAndClass() 
                ...
                + ' style="font-size:' + this.dom.css('font-size')  + ';height:.9em"/>');
	...
	setInitialValue: function() {
		...
		var editor = this.dom.find(':text, textarea');
	...
	handleCancelEditor: function(anEvent) {
        ...
        var enteredText = this.dom.find('input, textarea').val(); // TW
		enteredText = this.triggerDelegateCall('willCloseEditInPlace', enteredText);
		
		this.restoreOriginalValue();
		if (0 && hasContent(enteredText) // TW
	...
	handleSubmitToServer: function(enteredText) {
		...
		this.didStartSaving();
		var that = this,
        URL = $.isFunction(that.settings.url) ? that.settings.url(that.settings): that.settings.url;
