...
function auth_after_success (req, res){
    ...
    box.invoke('openID.afterAuth', req, function(err, Saved){
        ...
        if( 1 || req.user && ( req.user.email || req.user.emailPinged )){
            ...
            res.redirect( referer || 'http://127.0.0.1:3000/colls'  ); // config.passport_after.userHasEmail
        }else{
...
box.on('init',  function( App, Config, initDone ){
    ...
    async.map( config.passports, setPassport, function(err, result){
        ...
        initDone(null,  '+' + ( new Date().getTime() - ts) + "ms Passport's OpenIDs initialised [" + result.join(', ') + ']');
    } );
});
