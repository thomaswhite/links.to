...
function scrape_body( $, uri, callback ){
    ...
    for( i=0; i< aH1.length; i++ ){
        body.h1.push( $( aH1[i]).text().replace(/(\n+|\t+|\s\s+)/g, ' ').trim() );
    }
    for( i=0; i< aH2.length; i++ ){
        body.h2.push( $( aH2[i]).text().replace(/(\n+|\t+|\s\s+)/g, ' ').trim() );
    }
...
//              bodyHTML: async.apply( bodyHTML, $ )
            ...
            function(err, result){
                ...
                result.url = url;
                result.rawHTML = html_page_body;
                result.ready = true;
                result.canonical = result.head.links.canonical || result.head.twetter.url;
                callback(err, result );
            }
        );
    });
