...
function removeJobs( aJobs, Done){
    if( aJobs && aJobs.length ){
        async.mapLimit(aJobs, 10,
            function( id, done ){
                kue.Job.get(id, function(err, job){
                    if( !job ){
                        done(null, id + ' is missing.');
                    }else{
        ...
        Done( null, 'Delete empty job list?' );
    }
}
...
box.on('init', function (App, Config, done) {
    ...
    async.map(
        ['inactive', 'complete', 'failed', 'active'],
        function(type, mapDone){
            jobs[type](function(err, aJobs){
                if( err ){
                    debug("Error during deleting jobs of type %s %j", type, err  );
                    mapDone(err);
                }else{
                    debug('Deleting %d "%s" jobs:', aJobs.length, type);
                    removeJobs( aJobs, function(err, results){
                        debug("%s, %j", type, results);
                        mapDone(null, results);
                    });
                }
            });
        },
        function(err, results){
            debug('All old jobs were deleted.');

            var jobs_id = [],
                jobs_processors = request_directory.get( path.join( Config.__dirname, config.jobs_dir ) )
                ;
           ...
           done( null, '+' + ( new Date().getTime() - ts) + 'ms plugin "KUE" initialised. Jobs registered:[' + jobs_id.join(', ') + ']');
        }
    );
